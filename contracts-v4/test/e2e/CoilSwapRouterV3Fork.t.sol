...
///
///   Provide a token that has a v3 pool with liquidity (e.g. one of your instant-V3 launches --
///   those carry no transfer tax, which v3 requires) and the chain's WETH. Set V3_FEE_TIER to the
///   token's pool tier (100 / 500 / 3000 / 10000; default 10000 = 1%):
///     V3_TEST_TOKEN=0x..  WETH_ADDRESS=0x..  V3_FEE_TIER=3000  \
...
contract CoilSwapRouterV3ForkTest is Test {
    ...
    address token;
    address weth;
    uint24 poolFee; // v3 fee tier of the test token's pool (V3_FEE_TIER env, default 10000)
    ...
    function setUp() public {
        ...
        poolFee = uint24(vm.envOr("V3_FEE_TIER", uint256(10_000)));
    ...
    function test_Fork_Buy_TakesInterfaceFee() public {
        ...
        vm.prank(alice);
        uint256 out = router.buy{value: ethIn}(token, poolFee, 0, alice, block.timestamp + 1);
    ...
    function test_Fork_Sell_TakesInterfaceFee() public {
        ...
        vm.deal(alice, ethIn);
        vm.prank(alice);
        uint256 bought = router.buy{value: ethIn}(token, poolFee, 0, alice, block.timestamp + 1);
        ...
        vm.prank(alice);
        uint256 ethOut = router.sell(token, poolFee, sellAmt, 0, alice, block.timestamp + 1);
